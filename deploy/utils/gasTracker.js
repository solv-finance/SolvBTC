...
const getGasPrice = (network) => {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (network == 'mainnet') {
      const response = JSON.parse(await get(`https://api.etherscan.io/api?module=gastracker&action=gasoracle&apikey=${apiKey}`));
      const currentGasPrice = Number(response["result"]["FastGasPrice"]).toFixed(0);
      if (currentGasPrice > highest) {
        reject(`gasPrice ${currentGasPrice} gwei too high, abort, current highest ${highest} , use export gas_price_highest=xxx to set`)
      }
