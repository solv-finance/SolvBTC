...
const transferAdminAndOwner = async (productName, token) => {
  console.log(`Start handling tasks for ${colors.yellow(productName)} - ${token.address} on ${colors.yellow(network.name)}`);
  ...
  console.log(`* ${colors.yellow(productName)}: Ownership has been transferrd to ${newAdmin}`);
}
