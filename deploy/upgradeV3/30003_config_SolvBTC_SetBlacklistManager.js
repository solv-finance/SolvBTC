...
module.exports = async ({ getNamedAccounts, network }) => {
  ...
  const SolvBTCFactory = await ethers.getContractFactory('SolvBTCV3_1', deployer);
  const solvBTCAddress = require('../SolvBTC/10099_export_SolvBTCInfos').SolvBTCInfos[network.name].erc20;
  ...
  if (currentManager == blacklistManager.address) {
    ...
    console.log(`* INFO: SolvBTC set blacklist manager to ${blacklistManager.address} at ${setManagerTx.hash}`);
