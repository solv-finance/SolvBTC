...
module.exports = async ({ getNamedAccounts, deployments, network }) => {
  ...
  const instance = await deploy('SolvBTCYieldToken' + version, {
    ...
    from: deployer,
    log: true,
    deterministicDeployment: ethers.utils.keccak256(ethers.utils.toUtf8Bytes('SolvBTCYieldToken' + deterministicSuffix[network.name]))
  });
  console.log(`* INFO: ${colors.yellow(`SolvBTCYieldTokenImpl`)} deployed at ${colors.green(instance.address)} on ${colors.red(network.name)}`);
