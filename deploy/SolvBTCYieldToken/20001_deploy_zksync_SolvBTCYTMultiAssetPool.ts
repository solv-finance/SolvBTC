...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) { 
  ...
  const artifact = await hre.artifacts.readArtifact("SolvBTCYieldTokenMultiAssetPool");
  const multiAssetPoolFactory = await hre.ethers.getContractFactory("SolvBTCYieldTokenMultiAssetPool");

  const multiAssetPool = await hre.upgrades.deployProxy(
    multiAssetPoolFactory, [], { initializer: 'initialize' }
  );
  await multiAssetPool.waitForDeployment();

  const multiAssetPoolAddress = await multiAssetPool.getAddress();
  console.log("SolvBTCYieldTokenMultiAssetPool proxy deployed to:", multiAssetPoolAddress);
  ...
  const implAddressHex = await hre.ethers.provider.getStorage(multiAssetPoolAddress, IMPLEMENTATION_SLOT);
