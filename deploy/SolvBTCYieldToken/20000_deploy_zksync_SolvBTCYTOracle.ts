...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) { 
  ...
  const artifact = await hre.artifacts.readArtifact("SolvBTCYieldTokenOracleForSFT");
  const oracleFactory = await hre.ethers.getContractFactory("SolvBTCYieldTokenOracleForSFT");

  const oracle = await hre.upgrades.deployProxy(
    oracleFactory, [], { initializer: 'initialize' }
  );
  await oracle.waitForDeployment();

  const oracleAddress = await oracle.getAddress();
  console.log("SolvBTCYieldTokenOracleForSFT proxy deployed to:", oracleAddress);
  ...
  const implAddressHex = await hre.ethers.provider.getStorage(oracleAddress, IMPLEMENTATION_SLOT);
