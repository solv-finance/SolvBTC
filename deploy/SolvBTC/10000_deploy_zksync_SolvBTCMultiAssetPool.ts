...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) { 
  ...
  const solvBTCMultiAssetPoolFactory = await hre.ethers.getContractFactory("SolvBTCMultiAssetPool");

  const solvBTCMultiAssetPool = await hre.upgrades.deployProxy(
    solvBTCMultiAssetPoolFactory, [], { initializer: 'initialize' }
  );
  await solvBTCMultiAssetPool.waitForDeployment();

  const solvBTCMultiAssetPoolAddress = await solvBTCMultiAssetPool.getAddress();
  console.log("SolvBTCMultiAssetPool proxy deployed to:", solvBTCMultiAssetPoolAddress);
  ...
  const implAddressHex = await hre.ethers.provider.getStorage(solvBTCMultiAssetPoolAddress, IMPLEMENTATION_SLOT);
