...
module.exports = async ({ getNamedAccounts, deployments, network }) => {
  ...
  const instance = await deploy('SolvBTC' + version, {
    ...
    from: deployer,
    log: true,
    deterministicDeployment: ethers.utils.keccak256(ethers.utils.toUtf8Bytes('SolvBTC' + deterministicSuffix[network.name]))
  });
  console.log(`* INFO: ${colors.yellow(`SolvBTCImpl`)} deployed at ${colors.green(instance.address)} on ${colors.red(network.name)}`);
