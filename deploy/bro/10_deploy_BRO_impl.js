...
module.exports = async ({ getNamedAccounts, deployments, network }) => {
  ...
  const instance = await deploy('BROImpl' + version, {
    ...
    from: deployer,
    log: true,
    deterministicDeployment: ethers.utils.keccak256(ethers.utils.toUtf8Bytes("BRO" + deterministicSuffix))
  });
  console.log(`* INFO: ${colors.yellow(`BROImpl`)} deployed at ${colors.green(instance.address)} on ${colors.red(network.name)}`);
};
