    ...
    GovernorControlUpgradeable
{
    ...
    event SetSftWrappedTokenFactory(
        address indexed previousSftWrappedTokenFactory,
        address indexed newSftWrappedTokenFactory
    );
    ...
    function initialize(
        ...
        address sftWrappedTokenFactory_, 
        ...
        external initializer
    {
        ...
        _setSftWrappedTokenFactory(sftWrappedTokenFactory_);
        ...
        returns (uint256 toSftId_)
    {
        ...
        if (
            ...
            swtAddress_ == ISolvBTCMultiAssetPool(solvBTCMultiAssetPool).getSolvBTC(sft_, slot_)
        ) {
        ...
        returns (uint256 redemptionId_)
    {
        ...
        if (ISolvBTCMultiAssetPool(solvBTCMultiAssetPool).isSftSlotAllowed(address(openFundShare), openFundShareSlot)) {
            sftWrappedToken = ISolvBTCMultiAssetPool(solvBTCMultiAssetPool).getSolvBTC(address(openFundShare), openFundShareSlot);
        ...
        internal virtual returns (address sftWrappedToken) 
    {
        if (ISolvBTCMultiAssetPool(solvBTCMultiAssetPool).isSftSlotAllowed(sft_, slot_)) {
            sftWrappedToken = ISolvBTCMultiAssetPool(solvBTCMultiAssetPool).getSolvBTC(sft_, slot_);
    ...
    function setSftWrappedTokenFactory(address sftWrappedTokenFactory_) external virtual onlyAdmin {
        _setSftWrappedTokenFactory(sftWrappedTokenFactory_);
    }

    function _setSftWrappedTokenFactory(address sftWrappedTokenFactory_) internal virtual {
        require(sftWrappedTokenFactory_ != address(0), "SftWrapRouter: invalid sftWrappedTokenFactory");
        emit SetSftWrappedTokenFactory(sftWrappedTokenFactory, sftWrappedTokenFactory_);
    ...
    function _setSolvBTCMultiAssetPool(address solvBTCMultiAssetPool_) internal virtual {
        require(solvBTCMultiAssetPool_ != address(0), "SftWrapRouter: invalid solvBTCMultiAssetPool");
