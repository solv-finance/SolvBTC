...
contract SolvBTCMultiAssetPoolTest is Test {
    ...
    address internal constant SOLVBTC_HOLDER_1 = 0xB2043c543bb60Fb7AA7265CdcA359A35C4Bc09fa;
    address internal constant SOLVBTC_HOLDER_2 = 0xEae8F93CafB5A280Cc0060fB1733d6A28a81BC14;
    ...
    uint256 internal constant GMXBTCA_SLOT = 70120449060830493694329237233476283664004495384113064083934107225342856266913;
    uint256 internal constant GMXBTCB_SLOT = 18834720600760682316079182603327587109774167238702271733823387280510631407444;
    uint256 internal GMXBTCA_HOLDING_VALUE_SFT_ID;
    uint256 internal GMXBTCB_HOLDING_VALUE_SFT_ID;

    uint256 internal constant GMXBTCA_SFT_ID_1 = 3043;
    address internal constant GMXBTCA_SFT_HOLDER_1 = 0xf65A8D44c6EC9823Ff7BE42b3CAc3b457aE8D520;
    uint256 internal constant GMXBTCA_SFT_ID_2 = 3585;
    address internal constant GMXBTCA_SFT_HOLDER_2 = 0xB2043c543bb60Fb7AA7265CdcA359A35C4Bc09fa;

    uint256 internal constant GMXBTCB_SFT_ID_1 = 369;
    address internal constant GMXBTCB_SFT_HOLDER_1 = 0x1b7cF985eC878328387a5c4e593AC595aD31e643;
    uint256 internal constant GMXBTCB_SFT_ID_2 = 371;
    address internal constant GMXBTCB_SFT_HOLDER_2 = 0xc98E728b0B0Cb00afaECf5B201CAC3299100d574;

    uint256 internal constant SOLVBTCENA_SLOT = 73370673862338774703804051393194258049657950181644297527289682663167654669645;
    ...
    function test_InitialStatus() public {
        ...
        assertFalse(solvBTCMultiAssetPool.isSftSlotAllowed(SOLVBTC_SFT, SOLVBTC_SLOT));
        assertFalse(solvBTCMultiAssetPool.isSftSlotAllowed(FUND_SFT, GMXBTCA_SLOT));
        assertFalse(solvBTCMultiAssetPool.isSftSlotAllowed(FUND_SFT, GMXBTCB_SLOT));
    }

    /** Tests for add/remove sft slots */
    function test_AddSftSlot() public {
        _addDefaultSftSlots();
        assertTrue(solvBTCMultiAssetPool.isSftSlotAllowed(SOLVBTC_SFT, SOLVBTC_SLOT));
        assertTrue(solvBTCMultiAssetPool.isSftSlotAllowed(FUND_SFT, GMXBTCA_SLOT));
        assertTrue(solvBTCMultiAssetPool.isSftSlotAllowed(FUND_SFT, GMXBTCB_SLOT));
        assertFalse(solvBTCMultiAssetPool.isSftSlotAllowed(FUND_SFT, SOLVBTC_SLOT));
        assertFalse(solvBTCMultiAssetPool.isSftSlotAllowed(FUND_SFT, SOLVBTCENA_SLOT));

        assertEq(solvBTCMultiAssetPool.getSolvBTC(SOLVBTC_SFT, SOLVBTC_SLOT), address(solvBTC));
        assertEq(solvBTCMultiAssetPool.getSolvBTC(FUND_SFT, GMXBTCA_SLOT), address(gmxBTC));
        assertEq(solvBTCMultiAssetPool.getSolvBTC(FUND_SFT, GMXBTCB_SLOT), address(gmxBTC));
        ...
        assertEq(solvBTCMultiAssetPool.getSftSlotBalance(SOLVBTC_SFT, SOLVBTC_SLOT), IERC3525(SOLVBTC_SFT).balanceOf(SOLVBTC_HOLDING_VALUE_SFT_ID));
        assertEq(solvBTCMultiAssetPool.getHoldingValueSftId(FUND_SFT, GMXBTCA_SLOT), 0);
        assertEq(solvBTCMultiAssetPool.getHoldingValueSftId(FUND_SFT, GMXBTCB_SLOT), 0);
    }
    ...
    function test_RemoveSftSlot() public {
        _addDefaultSftSlots();
        vm.startPrank(ADMIN);
        solvBTCMultiAssetPool.removeSftSlotOnlyAdmin(FUND_SFT, GMXBTCB_SLOT);
        assertTrue(solvBTCMultiAssetPool.isSftSlotAllowed(SOLVBTC_SFT, SOLVBTC_SLOT));
        assertTrue(solvBTCMultiAssetPool.isSftSlotAllowed(FUND_SFT, GMXBTCA_SLOT));
        assertFalse(solvBTCMultiAssetPool.isSftSlotAllowed(FUND_SFT, GMXBTCB_SLOT));
        vm.stopPrank();
    ...
    function test_RevertWhenRemoveSftSlotByNonAdmin() public {
        _addDefaultSftSlots();
        vm.startPrank(USER);
        vm.expectRevert("only admin");
        solvBTCMultiAssetPool.removeSftSlotOnlyAdmin(FUND_SFT, GMXBTCB_SLOT);
    ...
    function test_RevertWhenRemoveInexistentSftSlot() public {
        vm.startPrank(ADMIN);
        vm.expectRevert("SolvBTCMultiAssetPool: sft slot not allowed");
        solvBTCMultiAssetPool.removeSftSlotOnlyAdmin(SOLVBTC_SFT, SOLVBTC_SLOT);
        vm.stopPrank();
    }

    function test_RevertWhenRemoveNonEmptySftSlot() public {
        _addDefaultSftSlots();
        vm.startPrank(ADMIN);
        vm.expectRevert("SolvBTCMultiAssetPool: holdingValueSftId balance not empty");
        solvBTCMultiAssetPool.removeSftSlotOnlyAdmin(SOLVBTC_SFT, SOLVBTC_SLOT);
        vm.stopPrank();
    }
    ...
    function test_RevertWhenWithdrawValueExceedsBalance() public {
        ...
        vm.expectRevert(abi.encodeWithSignature("ERC20InsufficientBalance(address,uint256,uint256)", SOLVBTC_HOLDER_1, withdrawValue, withdrawValue + 1));
    ...
    function test_RevertWhenWithdrawButPoolHoldingValueNotEnough() public {
        ...
        vm.mockCall(address(solvBTC), abi.encodeWithSignature("balanceOf(address)", SOLVBTC_HOLDER_1), abi.encode(solvBTCSftHoldingValue + 1));
        uint256 withdrawValue = solvBTC.balanceOf(SOLVBTC_HOLDER_1);
        solvBTC.approve(address(solvBTCMultiAssetPool), withdrawValue);
        vm.expectRevert("SolvBTCMultiAssetPool: insufficient balance");
        solvBTCMultiAssetPool.withdraw(SOLVBTC_SFT, SOLVBTC_SLOT, 0, withdrawValue);
    ...
    function test_RevertWhenWithdrawWithMismatchedSpecifiedSlot() public {
        ...
        solvBTC.approve(address(solvBTCMultiAssetPool), withdrawValue);
        vm.expectRevert("SolvBTCMultiAssetPool: slot not matched");
        solvBTCMultiAssetPool.withdraw(FUND_SFT, GMXBTCB_SLOT, 2850, withdrawValue);
        vm.stopPrank();
    }
